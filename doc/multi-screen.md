...
This sim was scaffolded by `npm run scaffold-screens` into `src/plate-tectonics/` (fleet naming:
...
1. Copies the `plate-tectonics/` prototype into `src/<kebab>/` per screen
...
4. Removes the prototype `plate-tectonics/` folder
...
main.ts
  └─ PlateTectonicsScreen            (Screen<PlateTectonicsModel, PlateTectonicsScreenView>)
       ├─ PlateTectonicsModel         owns all state
       └─ PlateTectonicsScreenView    owns all visuals
```
...
template's `PlateTectonicsScreen.ts`. Screen icons live in one shared module under
...
```
src/common/PlateTectonicsScreenIcons.ts
```
